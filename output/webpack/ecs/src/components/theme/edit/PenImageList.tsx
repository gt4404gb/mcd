      ...
      <div className="info_imagesList">
                      ...
                      rules={[
                        ({ getFieldValue }) => ({
                          validator(_, value) {
                            ...
                            if (!value || (value && !getFieldValue('enPenImgList')[index].obj || !getFieldValue('enPenImgList')[index].obj.url)) {
                              return Promise.reject(new Error('请上传钢笔图或删除图片占位!'));
                            }
                    ...
                    <Form.Item name={[field.name, 'obj']}
                      rules={[
                        ({ getFieldValue }) => ({
                          validator(_, value) {
                            /* @ts-ignore */
                            let index = _.field.slice(11, 12);
                            if (!value || (value && !getFieldValue('penImgList')[index].obj || !getFieldValue('penImgList')[index].obj.url)) {
                              return Promise.reject(new Error('请上传钢笔图或删除图片占位!'));
                            }
                            return Promise.resolve();
                          },
        ...
        {imagesNum > 1 && <div className="change-pic">
          <Button type="link" onClick={() => {
            let images = style === 'en' ? form.getFieldValue('enPenImgList') : form.getFieldValue('penImgList');
            let res = images.some((item: any) => {
              return !item || !item.obj || !item.obj.url
            })
            if (res) {
              message.error('请上传钢笔图或删除图片占位!');
              return
            }
            setList(images);
