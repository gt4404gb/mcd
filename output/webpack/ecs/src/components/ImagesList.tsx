...
import { Form, Button, message } from '@aurum/pfe-ui';
// @ts-ignore
      ...
      <div className="info_imagesList">
                    ...
                    <Form.Item name={[field.name, 'obj']}
                      rules={[
                        ({ getFieldValue }) => ({
                          validator(_, value) {
                            /* @ts-ignore */
                            let index = _.field.slice(14, 15);
                            if (!value || (value && !getFieldValue('basicImagesEn')[index].obj || !getFieldValue('basicImagesEn')[index].obj.fileUrl)) {
                    ...
                    <Form.Item name={[field.name, 'obj']}
                      rules={[
                        ({ getFieldValue }) => ({
                          validator(_, value) {
                            /* @ts-ignore */
                            let index = _.field.slice(12, 13);
                            if (!value || (value && !getFieldValue('basicImages')[index].obj || !getFieldValue('basicImages')[index].obj.fileUrl)) {
                              return Promise.reject(new Error('请上传商品图或删除图片占位!'));
                            }
                            return Promise.resolve();
                          },
        ...
        {imagesNum > 1 && <div className="change-pic">
          <Button type="link" onClick={() => {
            let images = style === 'en' ? form.getFieldValue('basicImagesEn') : form.getFieldValue('basicImages');
            let res = images.some((item: any) => {
              return !item || !item.obj || !item.obj.fileUrl
            })
            if (res) {
