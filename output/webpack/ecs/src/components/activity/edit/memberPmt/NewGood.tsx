...
export default connect(mapStateToProps, mapDispatchToProps)(({ activityReward, canOnlyView = false, showVisible, scene, onClose, SetRefresh }: any) => {
  ...
  const [searchObj, setSearchObj]: any = useState({ ...initSearchObj, activId: activityId, shopIds: '主题活动' });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (filterObj && filterObj.categories && filterObj.categories.length > 0) {
        ...
        setSearchObj({ ...searchObj, catRuleIds: defaultRuleId, shopIds: '主题活动' });
  ...
  useEffect(() => {
    setSearchObj({ ...searchObj, spuIds: [], name: '', shopIds: '主题活动' })
  }, [showVisible])
  ...
  useEffect(() => {
    ...
    form.resetFields();
    (async () => {
      const searchConds = { ...searchObj, shopIds: [5], activId: activityId };
  ...
  const toBind = () => {
    ...
    if (activityReward.spuList.length >= 1) {
      message.error('最多添加1个商品')
      return;
    }
    if (scene === 5) {
  ...
  const toSearch = () => {
    ...
    setSearchObj({ ...searchObj, shopIds: '主题活动', ...narrowSearchObj });
  }
        ...
        <Form.Provider
          onFormFinish={(name, { values, forms }) => {
            if (name === 'proListNewForm') {
              ...
              if (activityReward.spuList.length + lists.length > 1) {
                message.error('最多添加1个商品')
                return;
              }
              let spuIds: any = [], bindSpuIds: any = [], glbalSpuIds = [], paramsLists: any = [];
              ...
              <Row gutter={16}>
                ...
                <Col span={3}>
                  <Form.Item label={$t('选择店铺')} name="shopIds" >
                    <Select placeholder={$t('不限')} options={shopSOptions} disabled={shopSOptions?.length == 1} />
                  </Form.Item>
                </Col>
              </Row>
              <Row gutter={16}>
                <Col span={6}>
                  <Form.Item>
                    <Space>
                      ...
                      {!activityRows?.length && <Link style={{ fontSize: 12 }} key="action-edit" to={'/ecs/merchants/edit?5'}>还没有商品,去新建</Link>}
                    </Space>
                  </Form.Item>
                </Col>
              </Row>
