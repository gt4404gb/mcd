...
import { Row, Col, Steppers, Button, Modal, Breadcrumb, Space } from '@aurum/pfe-ui';
const { Stepper } = Steppers;
import * as activityActions from '@/redux/actions/activityActions';
import ActivityEditInfo from '@/components/activity/edit/Info';
import ActivityEditProPanel from '@/components/activity/edit/ProductPanel';
//import ActivityEditPro from '@/components/activity/edit/reward/ProductNew';
  ...
  return <>
    ...
    <div id="activity-edit" className="activity-edit">
      <div className="inner-container" >
        <ActivityEditInfo STEP={0} onActionCompleted={(result: any) => {
          if (result) {
            gotoNexStep(1);
            if (result.canOnlyView) {
              history.push('/ecs/activity/edit/' + result.activityId + '/isShow');
            } else {
              history.push('/ecs/activity/edit/' + result.activityId);
            }
          }
        }} />
        <ActivityEditProPanel
          /* @ts-ignore */
          STEP={1}
