...
export default ({ onChange = null, value = '', disabled = false, uploadPath = 'gallery', allowedMimes = [], maxBytes = 0, onUploaded = null }: any) => {
  ...
  return (
    <div className="aurum-widegts-image-field" onMouseOver={() => {
      ...
      <Upload
        ...
        {...config.getUploadProps({
          ...
          onUploadCompleted: (imageObj: any) => {
            let finalImagesList = { fileUrl: imageObj.imageUrl, url: imageObj.imageUrl, height: imageObj.height, width: imageObj.width };
            setImageUrl(finalImagesList);
            if (onUploaded) {
              onUploaded(imageObj);
            } else {
              if (onChange) onChange(finalImagesList);
            }
          },
        ...
        {(imageUrl.fileUrl || imageUrl.url) ? <><img src={imageUrl.fileUrl || imageUrl.url} alt="上传的图片" style={{ width: '100%', height: '100%' }} />
          {uploadButton}
        </>
      ...
      >
        <img alt="example" style={{ width: '100%' }} src={imageUrl.fileUrl || imageUrl.url} />
      </Modal>
    </div>
  )
};