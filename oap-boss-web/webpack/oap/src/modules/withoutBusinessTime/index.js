...
import { APPLY_STATUS, FIELD_NAMES_DEFAULT } from '@/constants';
...
export default class withputBureport extends React.Component {
	constructor(props) {
		super(props)
		this.formWithoutBusinessTimeRef = React.createRef();
		this.state = {
			...
			columns: [
				...
				{ title: '业务域', dataIndex: 'name', width: 100 },
	...
	//重置查询条件
	onReset = () => {
		this.formWithoutBusinessTimeRef.current.resetFields();
	}
	...
	onPageChange = (pageNo, pageSize) => {
		this.setState({
			...
			this.formWithoutBusinessTimeRef.current.submit();
		});
	}
	...
	//获取查询列表
	fetchDataList = () => {
		let formData = this.formWithoutBusinessTimeRef.current.getFieldsValue();
		let commitParams = {
	...
	handleButton = (record) => {
		let arr = [
			{
				name: "详情",
				onClickEvent: () => this.linkToForm('detail', record)
			}
		]
	...
	linkToForm = (type, record) => {
		if (type === 'create') {
			this.props.history.push({
				pathname: "/oap/process-details/create",
	...
	toSupplierApply = () => {
		this.props.history.push({
			...
			search: querystring.stringify({ type: "BusinessScreen" })
		});
	}
				...
				<Row className="oap-row oap-sql-row">
					...
					<Col className="table-container oap-sql-right">
						<Form
							...
							ref={this.formWithoutBusinessTimeRef}
							layout="vertical"
							size="middle"
