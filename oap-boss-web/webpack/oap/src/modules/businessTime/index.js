...
import { Spin, Form, Row, Col, Button, Input, Table, Pagination, Tooltip, Empty, Space, Modal, message, ConfigProvider, Popconfirm } from "@aurum/pfe-ui";
...
class businessTime extends React.Component {
	constructor(props) {
		super(props);
		this.formBuReportRef = React.createRef();
		this.state = {
			isLoading: false,
	...
	//重置查询条件
	onReset = () => {
		this.formBuReportRef.current.resetFields();
	}
	...
	onPageChange = (pageNo, pageSize) => {
		this.setState({
			...
			this.formBuReportRef.current.submit();
		});
	}
	...
	init = async () => {
		...
		try {
			const res = await Promise.all([
				getSubjectTreeData({
					reportRangeModule: "BusinessScreen",
					accessStatus: 1,
	...
	//获取查询列表
	fetchDataList = () => {
		let formData = this.formBuReportRef.current.getFieldsValue();
		let commitParams = {
	...
	toApplyReport = () => {
		this.props.history.push({
			pathname: `/oap/without-business-time`,
		});
	};
	...
	toReportDetails = (id, reportCode, name) => {
		...
		let _operatorId = "",
			_operatorName = "",
			_typeRange = "MCD"; // 2023-04-11 string => int
		if (currentUserInfo) {
		...
		const obj = {
			...
			reportRange: 4, // "BusinessScreen", // 企业报告-EnterpriseReport、BU报告-BUReport、个人报告-IndividualReport、业务大屏-BusinessScreen
			reportId: id,
			// reportName: name, // reportId,
...
export default connect(mapStateToProps)(businessTime);